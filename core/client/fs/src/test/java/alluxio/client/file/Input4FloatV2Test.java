...
public class Input4FloatV2Test extends Input4BaseV2Test {
  public int unitSize() {
    return Float.BYTES;
  }
  @Override
  ...
  public void singleReadPos() throws Exception {
    for (int i = 0; i < UNIT_LENGTH; i++) {
      Assert.assertEquals((float) i, (float) mTestStream.readFloat(i * unitSize()), 0);
  ...
  public void randomRead() throws Exception {
    ...
    for (int i = 0; i < time; i++) {
      ...
      Assert.assertEquals((float) p, (float) mTestStream.readFloat(p * unitSize()), 0);
    }
  }
