    ...
    if (stream != null) {
      ...
      if (cache) {
        ...
        try {
          ...
          int mCacheTier = mOptions.getOpenUfsBlockOptions(blockId).getCacheTier();
          Protocol.AsyncCacheRequest request =
                  Protocol.AsyncCacheRequest.newBuilder().setBlockId(blockId).setLength(blockLength)
                          .setOpenUfsBlockOptions(mOptions.getOpenUfsBlockOptions(blockId)).setCacheTier(mCacheTier)
